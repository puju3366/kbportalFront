...
import { createModule, getCategories, getProject, getPractice, getTeam, rbackEmployeeDataFetch } from '../redux/Crud';
...
const Addpost = () => {
    ...
    var logintoken: any = jwt_decode(token);

    let PracticeTokenData = logintoken.device_token.practice.toLowerCase();
    const capitalized = PracticeTokenData.charAt(0).toUpperCase() + PracticeTokenData.slice(1);
...
// const[userDetailsRBACK,setUserDetailsRBACK] = useState({})
...
//         setUserDetailsRBACK(employeeData.device_token)
//         console.log("105", employeeData)
//     })
//     .catch(err => {
//         console.log("err", err);
//     })
...
*/
    ...
    useEffect(() => {
        document.title = "DEVIT - Add Post";
        getCategoriesData();
        getProjectData();
        getPracticeData();
        getTeamData();
        setState(prev => ({ ...prev, practiceTag: capitalized.split(',') }))
        setState(prev => ({ ...prev, teamTag: logintoken.device_token.team.split(',') }))
     ...
     * @Author 
     * @Method handleSubmit
     * @Purpose To submit new kb post data
     * @param {e} 
     * @Since Oct 2022
    */
    ...
    const formik = useFormik({
        ...
        onSubmit: values => {
            let data = {
                ...
                created_by: logintoken.device_token.email,
                kb_assigner: logintoken.device_token.email,
                kb_approved_by: logintoken.device_token.reporting_manager_name,
                kb_expiry_date: state.endDate,
            };
            if(data !== null){
                createModule(data)
                .then((res: any) => {
                    const module = res.data.result;
                    toast.success(res.data.items.msg);
                    setTimeout(() => {
                        setState(intialState)
                        navigate("/lists/practice/Cloud")
                    }, 1900);
                })
            } else{
    ...
    const PracticeOptions: any = state.practice && state.practice.map((item: any) => ({
    ...
    let practiceData = PracticeOptions.filter((element: any, index: any) => {
                            ...
                            <div className="card-body pb-0">
                                <form action="m-0" className="form mb-5 fv-plugins-bootstrap5 fv-plugins-framework" method="post" id="kt_addpost_form" onSubmit={formik.handleSubmit}>
                                    ...
                                    <div className="d-flex flex-column col-xxl-3 mx-1 mb-5 fv-row fv-plugins-icon-container">
                                        ...
                                        <select className="form-select" id="category_id" name="category_id" data-control="select2" data-hide-search="true"
                                            data-placeholder="Select a layout" value={formik.values.category_id} onChange={formik.handleChange}> onBlur={formik.handleBlur}
                                    ...
                                    <div className="d-flex flex-column col-xxl-3 mx-1 mb-5 fv-row fv-plugins-icon-container">
                                        ...
                                        <select className="form-select" id="project_id" name="project_id" data-control="select2" data-hide-search="true" data-placeholder="Select a layout" value={formik.values.project_id} onChange={formik.handleChange} onBlur={formik.handleBlur}>
                                    ...
                                    <div className="row py-5">
                                        <div className="col-md-12">
                                            <div className="d-flex flex-wrap button-groups-reverse w-xs-50">
                                                ...
                                                <a href="javascript:void(0)" className="btn btn-outline btn-outline-solid btn-outline-primary" onClick={handleDraft} title="Save as Draft">Save as Draft</a>
                                                <a className="btn btn-outline btn-outline-solid btn-outline-primary previewbtn" id='preview' title="Preview" onClick={handleShowModal}>Preview</a>
                                                <Modal isOpen={state.modalIsOpen} className="modal-css" >
                                                    <div style={{ backgroundColor: "#eee" }} className="inner-modal">
                                                        <div className="card-body" style={{ backgroundColor: "#eee" }}>
                                                            ...
                                                            <hr />
                                                            <div className="mb-5 mt-5 postInfo modal-text-adjustment"
